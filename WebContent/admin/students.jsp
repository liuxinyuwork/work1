					...
					<c:when test="${not empty students }">
						<c:forEach items="${students }" var="student">
							<tr>
								...
								<td><span id="hidePwd">******</span> <span id="showPwd"
									style="display: none">${student.studentPwd }</span>
								...
								<td>
									<div class="btn-group">
										<button type="button" class="btn btn-info btn-sm"
											onclick="_iframe(1, 'student/${student.studentId }', 'students')">修改</button>
										<button type="button" class="btn btn-danger btn-sm"
											onclick="del('student/${student.studentId }')">删除</button>
									</div>
								</td>
							</tr>
						</c:forEach>
				...
				<ul class="pagination">
					...
					<c:forEach begin="${pageNow }" end="${pageNow+5 }" step="1"
						...
						<c:if test="${pageNo <= pageTotal }">
							...
							<c:if test="${pageNow != pageNo }">
								<li><a href="students?startPage=${pageNo }"
