	...
	<div style="text-align: center;">
			...
			<thead>
				<tr>
					<c:if test="${handAdd != null }">
						<th>已选: <span id="choosed" style="color: red;">${choosed }</span>
						</th>
					</c:if>
					<th>试题编号</th>
					<th>题目</th>
					...
					<c:when test="${not empty subjects }">
						<c:forEach items="${subjects }" var="subject">
							<tr id="tr-${subject.subjectId }">
								<c:if test="${handAdd != null }">
									<td><input type="checkbox" name="chooseSubject"
										id="${subject.subjectId }" /></td>
								</c:if>
								<td>${subject.subjectId }</td>
								<td><c:if test="${fn:length(subject.subjectName) > 8 }">
										${fn:substring(subject.subjectName, 0, 8) }
										<a href="#"
											onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a>
									</c:if> <c:if test="${fn:length(subject.subjectName) <= 8 }">
										${subject.subjectName }
									</c:if></td>
								<td><c:if test="${fn:length(subject.optionA) > 5 }"> ${fn:substring(subject.optionA, 0, 5) } <sub><a
											href="#"
											onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub>
									</c:if> <c:if test="${fn:length(subject.optionA) <= 5 }"> ${subject.optionA }</c:if>
								</td>
								<td><c:if test="${fn:length(subject.optionB) > 5 }"> ${fn:substring(subject.optionB, 0, 5) } <sub><a
											href="#"
											onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub>
									</c:if> <c:if test="${fn:length(subject.optionB) <= 5 }"> ${subject.optionB }</c:if>
								</td>
								<td><c:if test="${fn:length(subject.optionC) > 5 }"> ${fn:substring(subject.optionC, 0, 5) } <sub><a
											href="#"
											onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub>
									</c:if> <c:if test="${fn:length(subject.optionC) <= 5 }"> ${subject.optionC }</c:if>
								</td>
								<td><c:if test="${fn:length(subject.optionD) > 5 }"> ${fn:substring(subject.optionD, 0, 5) } <sub><a
											href="#"
											onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub>
									</c:if> <c:if test="${fn:length(subject.optionD) <= 5 }"> ${subject.optionD }</c:if>
								</td>
								...
								<td><c:if test="${handAdd == null }">
										<div class="btn-group">
											<button type="button" class="btn btn-info btn-sm"
												onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">修改</button>
											<button type="button"
												class="btn btn-danger btn-sm delSubject"
												id="${subject.subjectId }">删除</button>
										</div>
									</c:if></td>
							</tr>
						</c:forEach>
		...
		<div>
			<ul class="pagination">
				...
				<li><a
					href="subjects?startPage=1&handAdd=${handAdd }&examPaperId=${examPaperId }">首页</a></li>
				<c:if test="${pageNow-1 > 0 }">
					<li><a
						href="subjects?startPage=${pageNow-1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">上一页</a></li>
				</c:if>
				<c:forEach begin="${pageNow }" end="${pageNow+4 }" var="subPage">
					<c:if test="${subPage-5 > 0 }">
						<li><a
							href="subjects?startPage=${subPage-5 }&handAdd=${handAdd }&examPaperId=${examPaperId }">${subPage-5 }</a></li>
					</c:if>
				</c:forEach>
				<c:forEach begin="${pageNow }" end="${pageNow+5 }" step="1"
					var="pageNo">
					<!-- 当前页码小于总页数才能显示 -->
					<c:if test="${pageNo <= pageTotal }">
						<c:if test="${pageNow == pageNo }">
							<li class="active"><a
								href="subjects?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }">${pageNo }</a></li>
						</c:if>
						<c:if test="${pageNow != pageNo }">
							<li><a
								href="subjects?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }"
				...
				<c:if test="${pageNow+1 <= pageTotal }">
					<li><a
						href="subjects?startPage=${pageNow+1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">下一页</a></li>
				</c:if>
				<li><a
					href="subjects?startPage=${pageTotal }&handAdd=${handAdd }&examPaperId=${examPaperId }">尾页</a></li>
				<li><a>${pageNow }/${pageTotal }</a></li>
				<li>
					<div style="width: -1%; height: 100%; float: right;">
						<form action="subjects" id="scannerPageForm">
							<input id="scannerPage" type="text" name="startPage"
								handAdd="${handAdd }" examPaperId="${examPaperId }"
