	...
	<div style="text-align: center;">
		<form:form action="class" method="POST" modelAttribute="editClass">
			...
			<c:if test="${editClass.classId != null }">
				<b class="form-title">班级编号：</b>
				<form:input class="ipt" path="classId" readonly="readonly"
			...
			<form:input path="className" class="ipt" />
			<br />
			<br />
			<b class="form-title">所属年级：</b>
			<form:select path="grade.gradeId" items="${grades }"
				itemLabel="gradeName" itemValue="gradeId" class="sel"></form:select>
			<br />
			<br />
			<b class="form-title">班 主&nbsp;&nbsp;任：</b>
			<form:select path="teacher.teacherId" items="${teachers }"
				itemLabel="teacherName" itemValue="teacherId" class="sel"></form:select>
			<br />
			<br />
			<!-- 存放上一个班主任编号 -->
			<input type="hidden" value="${editClass.teacher.teacherId }"
				name="lastTeacher" />
