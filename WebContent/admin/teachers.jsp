					...
					<c:if test="${sessionScope.adminPower == 1 }">
						<th>操作 &emsp;
							<button type="button" class="btn btn-xs btn-info"
								onclick="_iframe(0, 'admin/teacheredit.jsp', 'teachers')">添加</button>
						</th>
					</c:if>
				</tr>
			</thead>
					...
					<c:when test="${not empty teachers }">
						<c:forEach items="${teachers }" var="teacher">
							<tr>
								...
								<td>
									<!-- 只有真正后台管理员才能查看教师后台密码，且重置 --> <c:if
										test="${sessionScope.adminPower == 1 }">
									...
									</c:if> <c:if test="${sessionScope.adminPower != 1 }">
										******
									</c:if>
								</td>
								<td><c:if test="${teacher.adminPower == 0 }">
										普通教职工
									</c:if> <c:if test="${teacher.adminPower == 1 }">
										<span style="color: red;">管理员</span>
									</c:if></td>
								<td><c:if test="${teacher.isWork == 0 }">
										否
									</c:if> <c:if test="${teacher.isWork == 1 }">
								...
								<td>
									<div class="btn-group">
										...
										<c:if
											test="${sessionScope.loginTeacher.teacherName == teacher.teacherName or sessionScope.adminPower == 1 }">
											<c:set value="1" var="flag"></c:set>
											<button type="button" class="btn btn-info btn-sm"
												onclick="_iframe(1,'teacher/${teacher.teacherId }', 'teachers')">修改</button>
										</c:if>
										<c:if test="${sessionScope.adminPower == 1}">
											<button type="button" class="btn btn-danger btn-sm"
												onclick="del('teacher/${teacher.teacherId }')">删除</button>
										</c:if>
									</div>
								</td>
							</tr>
					...
					<ul class="pagination">
						...
						<c:forEach begin="${pageNow }" end="${pageNow+5 }" step="1"
							...
							<c:if test="${pageNo <= pageTotal }">
								...
								<c:if test="${pageNow != pageNo }">
									<li><a href="teachers?startPage=${pageNo }"
