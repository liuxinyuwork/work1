				...
				<tr>
					...
					<th>操作
						<button type="button" class="btn btn-xs btn-info"
							onclick="_iframe(0, 'preAddExamPaper', 'examPapers')">添加</button>
					</th>
				</tr>
			</thead>
			<tbody>
					...
					<c:when test="${not empty examPapers }">
						<c:forEach items="${examPapers }" var="ep">
							<tr class="subject_info">
								...
								<td><c:if test="${ep.division == 0 }">
										未分科
									</c:if> <c:if test="${ep.division == 1 }">
										文科
									</c:if> <c:if test="${ep.division == 2 }">
										理科
									</c:if></td>
								<td><c:if test="${ep.examPaperEasy == 0 }">
										简单
									</c:if> <c:if test="${ep.examPaperEasy == 1 }">
										普通
									</c:if> <c:if test="${ep.examPaperEasy == 2 }">
										困难
									</c:if></td>
								...
								<td>
									<div class="btn-group">
										<button type="button" class="btn btn-info btn-sm"
											onclick="_iframe(1, 'examPaper/${ep.examPaperId }', 'examPapers')">修改</button>
										<button type="button" class="btn btn-danger btn-sm"
											onclick="del('examPaper/${ep.examPaperId }')">删除</button>
										<button type="button" id="${ep.examPaperId }"
											class="btn btn-info btn-sm examPaper-subjects">查看试题</button>
										<button type="button"
											onclick="examPaperAddSubjects(${ep.examPaperId })"
											class="btn btn-info btn-sm">添加试题</button>
										<button type="button"
											onclick="autoAddSubjects(${ep.examPaperId })"
											class="btn btn-info btn-sm">生成试题</button>
									</div>
								</td>
							</tr>
						</c:forEach>
