	...
	<div class="container" style="margin-top: 150px;">
		...
		<div style="width: 60%; height: 100%; float: left;">
				...
				<dl subject="1" style="width: 100%;">
					...
					<c:if test="${esms != null }">
						...
						<c:forEach items="${esms }" var="esm">
							<dd style="float: left; display: none; width: 100%;"
								index="${index }" id="${esm.subject.subjectId }">
								<div>
								...
								<div>
									...
									<c:if test="${esm.subject.subjectType == 0 }">
										<p class="aswer-p">
											<input class="aswer-option"
												...
												value="A" />&nbsp;<span class="subjectOption">${esm.subject.optionA }</span>
										</p>
										<p class="aswer-p">
											<input class="aswer-option"
												id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-B"
												type="radio" name="chooseRight-${esm.subject.subjectId }"
												value="B" />&nbsp;<span class="subjectOption">${esm.subject.optionB }</span>
										</p>
										<p class="aswer-p">
											<input class="aswer-option"
												id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-C"
												type="radio" name="chooseRight-${esm.subject.subjectId }"
												value="C" />&nbsp;<span class="subjectOption">${esm.subject.optionC }</span>
										</p>
										<p class="aswer-p">
											<input class="aswer-option"
												id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-D"
												type="radio" name="chooseRight-${esm.subject.subjectId }"
												value="D" />&nbsp;<span class="subjectOption">${esm.subject.optionD }</span>
										</p>
									</c:if>
									<%-- 多选 --%>
									<c:if test="${esm.subject.subjectType == 1 }">
										<p class="aswer-p subject-id-${esm.subject.subjectId }"
											...
											<input class="aswer-option"
												id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-A"
												type="checkbox" name="chooseRight-${esm.subject.subjectId }"
												value="A" />&nbsp;<span class="subjectOption">${esm.subject.optionA }</span>
										</p>
										<p class="aswer-p subject-id-${esm.subject.subjectId }"
											data-sid="${esm.subject.subjectId }" data-type="checkbox"
									...
									<c:if test="${esm.subject.subjectType == 2 }">
										<p class="aswer-p subject-id-${esm.subject.subjectId }"
											...
											data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
											<textarea class="aswer-textarea"
												name="chooseRight-${esm.subject.subjectId }"
												style="width: 100%; height: 100px;"
												onblur="textareaSubmit(this)"></textarea>
										</p>
									</c:if>
								</div>
								<%--获取 记录已选的试题信息 --%>
								<c:if test="${chooses != null }">
									<c:forEach items="${chooses }" var="choose">
										<c:if
											test="${choose.subject.subjectId == esm.subject.subjectId }">
											<c:if test="${esm.subject.subjectType == 0 }">
			...
			<div style="width: 100%; height: 20%; margin-top: 170px;">
				<button style="float: left;" id="preSubject" type="button"
					class="btn btn-default btn-lg">上一题</button>
				<button style="float: left; margin-left: 10px;" id="nextSubject"
					type="button" class="btn btn-default btn-lg">下一题</button>
			</div>
		</div>
		...
		<!-- 答题卡 -->
		<span id="beginTime" style="display: none;">${beginTime }</span> <span
			id="examTime" style="display: none;">${examTime }</span>
		<div style="width: 38%; float: right; margin-top: -61px;">
			<div style="width: 100%; height: 63px; text-align: center;">
				...
				<div
					style="float: right; width: 55%; height: 100%; line-height: 63px; text-align: left;">
					<a
						href="submit?studentId=${sessionScope.loginStudent.studentId }&examPaperId=${examPaperId }&classId=${classId }&gradeId=${gradeId }"
						type="button" class="btn btn-default btn-sm"
						onclick="return confirm('确定提交吗?')">提交</a>
					<%--隐藏表单，用于考试结束且考生未手动提交试卷 自动提交 --%>
					<form action="submit" method="post" style="display: none;">
						<input type="hidden"
							value="${sessionScope.loginStudent.studentId }" name="studentId" />
						<input type="hidden" value="${examPaperId }" name="examPaperId" />
						<input type="hidden" value="${classId }" name="classId" /> <input
							type="hidden" value="${gradeId }" name="gradeId" />
					</form>
					<span style="font-weight: 600;">剩余时间： <span id="lastTime"
						style="color: #00A06B; font-size: 16px; font-weight: 900;">
							<span id="time_min">${examTime }</span>" <span id="time_sec">00</span>'
					</span>
					</span>
				</div>
			</div>
					...
					<c:if test="${esms != null }">
						...
						<c:forEach items="${esms }" var="esm">
							<c:if test="${indexAswer == 1 }">
								<li style="background-color: red;">${indexAswer }</li>
							</c:if>
							<c:if test="${indexAswer != 1 }">
								<li>${indexAswer }</li>
							</c:if>
