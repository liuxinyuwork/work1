			...
			<div class="col-md-12 column">
				<div class="row">
					<div style="border-bottom: 2px dashed #CAE5E8; text-align: center;">
						<h2>${examPaperName }</h2>
						<p>
							姓名：<u> <c:if test="${studentName == null }">${sessionScope.loginStudent.studentName }</c:if>
								<%--后台教师查看 --%> <c:if test="${studentName != null }">${studentName }</c:if>
							</u> &emsp; 得分：<u>${score }</u> &emsp; 考试时间：<u>${ExamedPaper.examPaperTime}</u>
						</p>
					</div>
					<c:choose>
									...
									<li>
										<div style="border-bottom: 2px solid #FFF; margin-left: 30px;">
											...
											<c:forEach items="${views }" var="view">
												...
												<c:if
													test="${esm.subject.subjectId == view.subject.subjectId }">
													<c:set value="0" var="isChoose"></c:set>
													<p>
														选择答案:
														<%--答案选对 --%>
														<c:if
															test="${view.subject.rightResult == view.chooseResult }">
															<span style="color: green"><b>${view.chooseResult }</b></span>
														</c:if>
														<%--答案选错，同时显示正确答案 --%>
														<c:if
															test="${view.subject.rightResult != view.chooseResult }">
															<span style="color: red"> <b>${view.chooseResult }</b>
															</span>
															&emsp;
															正确答案: <span style="color: #F1AF00;"><b>${view.subject.rightResult }</b></span>
														</c:if>
														&emsp; 分值: <b style="color: green;">${view.subject.subjectScore }</b>
														&emsp; 难易程度: <b style="color: green;"> <c:if
																test="${view.subject.subjectEasy == 0 }">简单</c:if> <c:if
																test="${view.subject.subjectEasy == 1 }">普通</c:if> <c:if
																test="${view.subject.subjectEasy == 2 }">困难</c:if>
														</b>
													</p>
												</c:if>
											</c:forEach>
											...
											<c:if test="${isChoose != 0 }">
												<p>
													选择答案: <b style="color: red;">未做</b> &emsp; 分值: <b
														style="color: green;">${esm.subject.subjectScore }</b>
													&emsp; 难易程度: <b style="color: green;"> <c:if
															test="${esm.subject.subjectEasy == 0 }">简单</c:if> <c:if
															test="${esm.subject.subjectEasy == 1 }">普通</c:if> <c:if
															test="${esm.subject.subjectEasy == 2 }">困难</c:if>
													</b>
												</p>
											</c:if>
										</div>
