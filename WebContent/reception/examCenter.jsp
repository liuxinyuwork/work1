...
<%@page
	import="com.taohan.online.exam.service.impl.ExamHistoryPaperServiceImpl"%>
							...
							<c:forEach items="${examPlans }" var="ep">
								<div class="col-md-4">
									<div class="thumbnail">
										...
										<div class="caption">
											<h3>${ep.examPaper.examPaperName }</h3>
											<p>
												考试科目: ${ep.course.courseName } &emsp;&emsp; 考试时长: <span
													id="examPaperTime${ep.examPaper.examPaperId }">${ep.examPaper.examPaperTime }</span>
												分钟
											</p>
											<p>
												题目数量: ${ep.examPaper.subjectNum } &emsp;&emsp; 总分:
												${ep.examPaper.examPaperScore } &emsp;&emsp; 难易程度:
											...
											<p>
												<a class="btn btn-default btn-lg btn-block beginExam"
													id="${ep.examPaper.examPaperId }"
													href="begin?classId=${ep.clazz.classId }&examPaperId=${ep.examPaper.examPaperId }&studentId=${sessionScope.loginStudent.studentId }&examTime=${ep.examPaper.examPaperTime }&gradeId=${gradeId }&beginTime=${ep.beginTime }">进入考试</a>
											</p>
										</div>
									</div>
								</div>
